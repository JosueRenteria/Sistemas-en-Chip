...
;CodeVisionAVR C Compiler V3.49 Evaluation
...
_tabla7segmentos:
	...
	.DB  0x7F,0x6F,0x77,0x7F,0x39,0x3F,0x79,0x71
...
; 0000 0026 DDRA=(1<<DDA7) | (1<<DDA6) | (1<<DDA5) | (1<<DDA4) | (1<<DDA3) | (1<<DDA2) | (1<<DDA1) | (1<<DDA0);
	LDI  R30,LOW(255)
	OUT  0x1A,R30
; 0000 0027 // State: Bit7=1 Bit6=1 Bit5=1 Bit4=1 Bit3=1 Bit2=1 Bit1=1 Bit0=1
; 0000 0028 PORTA=(1<<PORTA7) | (1<<PORTA6) | (1<<PORTA5) | (1<<PORTA4) | (1<<PORTA3) | (1<<PORTA2) | (1<<PORTA1) | (1<<PORTA0);
	OUT  0x1B,R30
; 0000 0029 
...
; 0000 0032 DDRC=(0<<DDC7) | (0<<DDC6) | (0<<DDC5) | (0<<DDC4) | (0<<DDC3) | (0<<DDC2) | (0<<DDC1) | (0<<DDC0);
	OUT  0x14,R30
; 0000 0033 // State: Bit7=P Bit6=P Bit5=P Bit4=P Bit3=P Bit2=P Bit1=P Bit0=P
; 0000 0034 PORTC=(1<<PORTC7) | (1<<PORTC6) | (1<<PORTC5) | (1<<PORTC4) | (1<<PORTC3) | (1<<PORTC2) | (1<<PORTC1) | (1<<PORTC0);
	LDI  R30,LOW(255)
	OUT  0x15,R30
; 0000 0035 
...
; 0000 0038 DDRD=(0<<DDD7) | (0<<DDD6) | (0<<DDD5) | (0<<DDD4) | (0<<DDD3) | (0<<DDD2) | (0<<DDD1) | (0<<DDD0);
	LDI  R30,LOW(0)
	OUT  0x11,R30
; 0000 0039 // State: Bit7=P Bit6=P Bit5=P Bit4=P Bit3=P Bit2=P Bit1=P Bit0=P
; 0000 003A PORTD=(1<<PORTD7) | (1<<PORTD6) | (1<<PORTD5) | (1<<PORTD4) | (1<<PORTD3) | (1<<PORTD2) | (1<<PORTD1) | (1<<PORTD0);
	LDI  R30,LOW(255)
	OUT  0x12,R30
; 0000 003B 
...
_0x3:
; 0000 008A {
; 0000 008B variable1 = PIND&0x0f; //Enmascaramos los 4 bits menos significativos
	IN   R30,0x10
	ANDI R30,LOW(0xF)
	MOV  R5,R30
; 0000 008C variable2 = PINC&0x0f; //Enmascaramos los 4 bits menos significativos
	IN   R30,0x13
	ANDI R30,LOW(0xF)
	MOV  R4,R30
; 0000 008D //del puerto A ya que los demás no interesan.
; 0000 008E PORTB = tabla7segmentos[variable1];
	MOV  R30,R5
	LDI  R31,0
	...
	LPM  R0,Z
	OUT  0x18,R0
; 0000 008F PORTA = ~tabla7segmentos[variable2];
	MOV  R30,R4
	LDI  R31,0
	SUBI R30,LOW(-_tabla7segmentos*2)
	SBCI R31,HIGH(-_tabla7segmentos*2)
	...
	OUT  0x1B,R30
; 0000 0090 
; 0000 0091 //if (variable1>=16) //Si lo que leemos es mayor o igual de 10 que dibuje en el display una E de ERROR
; 0000 0092 //PORTB = 0x79;
; 0000 0093 
; 0000 0094 //if (variable2>=16) //Si lo que leemos es mayor o igual de 10 que dibuje en el display una E de ERROR
; 0000 0095 //PORTA = ~0x79;
; 0000 0096 }
	RJMP _0x3
; 0000 0097 }
_0x6:
